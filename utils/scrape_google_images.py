...
search_name = 'school_hallway_without_students'
page_number = 0
params = {
    "q": "school hallway without students",  # search query
    "tbm": "isch",  # image results
    "hl": "en",  # language of the search
    "gl": "us",  # country where search comes from
    "ijn": f'{page_number}'  # page number
}
...
def get_original_images():
    ...
    for metadata, thumbnail, original in zip(soup.select('.isv-r.PNCib.MSM1fd.BUooTd'), thumbnails, full_res_images):
        ...
        file_dir = f'/media/dcofer/Ubuntu_Data/active_shooter_defense/gun_dataset/detector/other/not_gun_specific/{search_name}/'
        file_path = file_dir + f'{search_name}_img_{page_number*100 + index}.jpg'
        try:
            urllib.request.urlretrieve(original, file_path)
        except Exception as e:
            print(f"error downloading file {page_number*100 + index}")
        index += 1
...
def serpapi_get_google_images():
    ...
    for index, image in image_results:
        print(f'Downloading {index} image...')
        opener=urllib.request.build_opener()
        opener.addheaders=[
            ('User-Agent',
             'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.3538.102 Safari/537.36 Edge/18.19582')]
        urllib.request.install_opener(opener)
